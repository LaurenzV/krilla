//! A minimal PDF/X-6p example using an external output-profile reference.
//!
//! PDF/X-6p (ISO 15930-9) is like PDF/X-6, but the output intent ICC profile
//! is referenced externally instead of being embedded. Based on PDF 2.0;
//! live transparency is permitted.
//!
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    document.set_metadata(
        Metadata::new()
            .title("PDF/X-6p Example".to_string())
    ...
    surface.set_fill(Some(Fill {
        paint: rgb::Color::new(90, 175, 220).into(),
        // PDF/X-6p allows live transparency (inherited from PDF/X-4/X-6 semantics).
        opacity: NormalizedF32::new(0.75).unwrap(),
        ..Default::default()
    }));
    ...
    let out = path::absolute("pdf_x6p.pdf")?;
