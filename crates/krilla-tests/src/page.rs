...
fn media_box_impl(d: &mut Document, media_box: Rect) {
    ...
    surface.draw_path(&rect_to_path(100.0, 0.0, 200.0, 100.0));
    surface.set_fill(Some(blue_fill(0.5)));
    surface.draw_path(&rect_to_path(0.0, 100.0, 100.0, 200.0));
    surface.set_fill(Some(purple_fill(0.5)));
